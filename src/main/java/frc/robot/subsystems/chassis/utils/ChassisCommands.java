...
public static Command createPathFollowingCommand(Command onTrajectoryEnd, boolean keepPosition, boolean resetPosition,  PathPoint... points) {
    if (points.length < 2)
        return null;
    var trajectory = PathPlanner.generatePath(ChassisConstants.PATH_CONSTRAINTS, Arrays.asList(points));
    return createPathFollowingCommand(trajectory, resetPosition, keepPosition, onTrajectoryEnd);
    }
